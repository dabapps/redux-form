...
const initializeState = (
  ...
  const initializeField = (path, src, dest) => {
    ...
    if (dotIndex >= 0 && (openIndex < 0 || dotIndex < openIndex)) {
      ...
      if (Array.isArray(srcArray)) {
        if (rest.length) {
          ...
          result[key] = srcArray.map((srcValue, srcIndex) =>
            makeEntry(
              srcValue,
              destArray && destArray[srcIndex] && destArray[srcIndex].value,
              overwriteValues
            )
          );
        }
      ...
      result[path] = makeEntry(
        src && src[path],
        dest && dest[path] && dest[path].value,
        overwriteValues
      );
    }
    return result;
  ...
  return fields.reduce(
    (accumulator, field) => initializeField(field, values, accumulator),
    { ...state }
  );
};
