...
describe('actions', () => {
  ...
  it('should create initialize action', () => {
    ...
    expect(initialize(data, fields, false)).toEqual({
      type: INITIALIZE,
      data,
      fields,
  ...
  it('should throw an error if initialize is not given a fields array', () => {
    expect(() => initialize({ a: 1, b: 2 }, undefined)).toThrow(
      /must provide fields array/
    );
    expect(() => initialize({ a: 1, b: 2 }, 'not an array')).toThrow(
      /must provide fields array/
    );
    expect(() => initialize({ a: 1, b: 2 }, { also: 'not an array' })).toThrow(
      /must provide fields array/
    );
  });
