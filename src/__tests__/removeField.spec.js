...
describe('removeField', () => {
  ...
  it('should not return same instance', () => {
    ...
    expect(removeField(fields, 'foo')).not.toBe(fields);
  });
  ...
  it('should remove a nested field', () => {
    expect(removeField({ foo: { rat: 'bar' }, dog: 42 }, 'foo.rat')).toEqual({
      dog: 42,
    });
  });

  it('should remove a nested field from root', () => {
    expect(removeField({ foo: { rat: 'bar' }, dog: 42 }, 'foo')).toEqual({
      dog: 42,
    });
  });
  ...
  it('should remove an array field', () => {
    expect(
      removeField({ foo: [{ rat: 'bar' }], dog: 42 }, 'foo[].rat')
    ).toEqual({ dog: 42 });
  });

  it('should remove a deep field', () => {
    expect(
      removeField({ foo: [{ rat: { pig: 'bar' } }], dog: 42 }, 'foo[].rat.pig')
    ).toEqual({ dog: 42 });
  });
