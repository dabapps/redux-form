...
describe('updateField', () => {
  ...
  it('should set value', () => {
    expect(updateField({}, { value: 'foo' }, true, undefined).value).toBe(
      'foo'
    );
    expect(
      updateField({ value: 'foo' }, { value: 'bar' }, true, undefined).value
    ).toBe('bar');
    expect(updateField({}, { value: 42 }, true, undefined).value).toBe(42);
    expect(
      updateField({ value: 3 }, { value: 42 }, true, undefined).value
    ).toBe(42);
  });

  it('should set pristine and dirty', () => {
    const result1 = updateField(
      {},
      { value: 'foo', initial: 'foo' },
      false,
      undefined
    );
    expect(result1.dirty).toBe(false);
    expect(result1.pristine).toBe(true);
    const result2 = updateField(
      {},
      { value: 'foo', initial: 'bar' },
      false,
      undefined
    );
  ...
  it('should set error from submit error', () => {
    expect(
      updateField({}, { submitError: 'foo' }, false, undefined).error
    ).toBe('foo');
    expect(
      updateField({}, { submitError: 'bar' }, false, undefined).error
    ).toBe('bar');
  });

  it('should set error from async error', () => {
    expect(updateField({}, { asyncError: 'foo' }, false, undefined).error).toBe(
      'foo'
    );
    expect(updateField({}, { asyncError: 'bar' }, false, undefined).error).toBe(
      'bar'
    );
  });
  ...
  it('should prioritize submit error over async error', () => {
    expect(
      updateField(
        {},
        { asyncError: 'fooAsync', submitError: 'fooSubmit' },
        false,
        undefined
      ).error
    ).toBe('fooSubmit');
    expect(
      updateField(
        {},
        { asyncError: 'barAsync', submitError: 'barSubmit' },
        false,
        undefined
      ).error
    ).toBe('barSubmit');
  ...
  it('should prioritize sync error over async error', () => {
    expect(
      updateField({}, { asyncError: 'fooAsync' }, false, 'fooSync').error
    ).toBe('fooSync');
    expect(
      updateField({}, { asyncError: 'barAsync' }, false, 'barSync').error
    ).toBe('barSync');
  });

  it('should prioritize sync error over submit error', () => {
    expect(
      updateField({}, { submitError: 'fooSubmit' }, false, 'fooSync').error
    ).toBe('fooSync');
    expect(
      updateField({}, { submitError: 'barSubmit' }, false, 'barSync').error
    ).toBe('barSync');
  });
  ...
  it('should set active', () => {
    ...
    expect(updateField({ active: false }, {}, true, undefined).active).toBe(
      true
    );
    expect(updateField({}, {}, false, undefined).active).toBe(false);
    expect(updateField({ active: true }, {}, false, undefined).active).toBe(
      false
    );
  });
  ...
  it('should set touched', () => {
    // init
    expect(updateField({}, { touched: true }, false, undefined).touched).toBe(
      true
    );
    expect(updateField({}, { touched: false }, false, undefined).touched).toBe(
      false
    );
    expect(updateField({}, {}, false, undefined).touched).toBe(false);
    // update
    expect(
      updateField({ touched: false }, { touched: true }, false, undefined)
        .touched
    ).toBe(true);
    expect(
      updateField({ touched: true }, { touched: false }, false, undefined)
        .touched
    ).toBe(false);
    expect(updateField({ touched: true }, {}, false, undefined).touched).toBe(
      false
  ...
  it('should set visited', () => {
    // init
    expect(updateField({}, { visited: true }, false, undefined).visited).toBe(
      true
    );
    expect(updateField({}, { visited: false }, false, undefined).visited).toBe(
      false
    );
    expect(updateField({}, {}, false, undefined).visited).toBe(false);
    // update
    expect(
      updateField({ visited: false }, { visited: true }, false, undefined)
        .visited
    ).toBe(true);
    expect(
      updateField({ visited: true }, { visited: false }, false, undefined)
        .visited
    ).toBe(false);
    expect(updateField({ visited: true }, {}, false, undefined).visited).toBe(
      false
  ...
  it('should change initial and default values when initial changes', () => {
    expect(
      updateField({ initialValue: 1 }, { initial: 2 }, false, undefined)
        .initialValue
    ).toBe(2);
    expect(
      updateField({ initialValue: 1 }, { initial: undefined }, false, undefined)
        .initialValue
    ).toBe(undefined);
  });
});
